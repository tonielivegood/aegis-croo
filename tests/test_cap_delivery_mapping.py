...
def test_decision_block_maps_correctly() -> None:
    ...
    body = json.loads(mapping.deliverable_schema)
...
def test_service_required_deliverable_fields_present() -> None:
    ...
    assert mapping.deliverable_type == "schema"
...
def test_delivery_json_is_deterministic() -> None:
    ...
    body_a = json.loads(mapping_a.deliverable_schema)
    body_b = json.loads(mapping_b.deliverable_schema)
    ...
    payload = json.loads(mapping_a.deliverable_schema)
    reserialized = json.dumps(payload, sort_keys=True, separators=(",", ":"), ensure_ascii=True)
    assert reserialized == mapping_a.deliverable_schema
